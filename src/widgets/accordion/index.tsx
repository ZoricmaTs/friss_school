...
export default function Accordions() {
  ...
  //     answer: 'Пропущенные занятия не восстанавливаются.',
  //   },
  //   {
  //     id: "2",
  ...
  //     answer: 'Всё необходимое оборудование и инструменты для шитья есть в школе. Вам нужно будет приобрести только ткани и фурнитуру для ваших изделий. Но до начала курса ничего покупать не нужно — преподаватели подробно расскажут, что понадобится, уже на первых занятиях.',
  //   },
  //   {
  //     id: "4",
  //     question: 'Где посмотреть расписание занятий в школе кроя и шитья?',
  //     answer: 'У нас регулярно формируются новые группы. Каждое занятие длится 3 часа и мы поможем подобрать удобное расписание обучения: утренние (9:00-12:00), дневные (13:00-16:00) или вечерние (16:00-19:00) группы. Занятия проходят три раза в неделю (пн-ср-пт или вт-чт-сб). Записаться можно по номеру телефона: +996 504 362 514 (звонок, WhatsApp, Telegram)\n',
  //   },
  // ];

  return <section className={'accordions-section'}>
    {dynamicStore.accordions.map((item: AccordionType, index: number) => <Accordion item={item} key={`accordion-${index}`}/>)}
  </section>
}