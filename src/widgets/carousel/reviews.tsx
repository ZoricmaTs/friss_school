...
export function CarouselReviews({items}: { items: CarouselItem[] }) {
  ...
  return <section className={'section-review'}>
    <div className={'carousel-review'} ref={rootRef}>
      <div className={'carousel-review__wrapper'} style={{
        width: `calc(${50 * (items.length)}vh + ${10 * (items.length - 1)}px)`,
        left: `calc(50vw - ${50 * activeIndex + 25}vh - ${10 * activeIndex}px)`,
      }}>
        {items.map((item, index) => {
          ...
          >
            ...
            <button className={'carousel-review__button'}>
              <p style={{paddingRight: '0.5rem'}}>{'Подробнее'}</p>
              <ArrowRightIcon size={24} className={'carousel-review__button_icon'}/>
            </button>
          </div>
        })}
      </div>
